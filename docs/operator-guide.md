...
2. Register the client's intake: business type, target public function,
   prior filing history in Germany if any.
3. Run the advisor in read-only mode against the e-Vergabe federal procurement platform, governed by the GWB/VgV (Vergaberecht).
...
- named referral relationship with Germany-licensed counsel or a
...
a working referral relationship with Germany-licensed counsel or a
registered agent for whatever licensed representation the law of
Germany requires for actual public-procurement filings.
