(ns marketentry.governor
  ...
  notion of jurisdictional procurement law, whether a Germany-resident
  ...
  actuation is always a human call.
    ...
    2. Evidence incomplete         -- for `:filing/draft`/
                                       ...
                                       checklist on file?
    3. Germany-resident-rep missing  -- for `:filing/submit`, when the
                                       engagement declares
                                       `:requires-local-rep?
                                       true` (near-universal for Germany
                                       ...
                                       Grounded in authorized representative /
                                       domicile requirements for non-EU bidders
                                       on certain procedures (Beschaffungsamt /
    ...
    (let [e (store/engagement st subject)
      ...
      (when-not (and assessment
        ...
        [{:rule :evidence-incomplete
          :detail "法域の必要書類(UST-IDNR/統一資格/e-Vergabe登録/代理人確認等)が充足していない状態での提案"}]))))

(defn- local-rep-missing-violations
  "For `:filing/submit`, when the engagement declares
  `:requires-local-rep? true`, INDEPENDENTLY verify
  `:has-local-rep?` is true -- the flagship genuinely new
  check this vertical adds. CONDITIONAL on the engagement's own
  `:requires-local-rep?` ground truth (most Germany public
  tenders require it; a pure-foreign-market engagement may not)."
  [{:keys [op subject]} st]
  (when (= op :filing/submit)
    (let [e (store/engagement st subject)]
      (when (and (true? (:requires-local-rep? e))
                 (not (true? (:has-local-rep? e))))
        [{:rule :local-rep-missing
          :detail (str subject " はドイツ居住代理人を要するが未確認 -- 提出提案は進められない")}]))))
...
(defn- ust-idnr-unverified-violations
  "For `:filing/submit`, when the engagement declares
  `:requires-ust-idnr? true`, INDEPENDENTLY check
  `:ust-idnr-verified?` -- CONDITIONAL on the engagement's own
  ground truth."
  [{:keys [op subject]} st]
  (when (= op :filing/submit)
    (let [e (store/engagement st subject)]
      (when (and (true? (:requires-ust-idnr? e))
                 (not (true? (:ust-idnr-verified? e))))
        [{:rule :ust-idnr-unverified
          :detail (str subject " はUST-IDNR確認を要するが未確認 -- 提出提案は進められない")}]))))
...
(defn check
  ...
  (let [hard (into []
                   (concat (spec-basis-violations request proposal)
                           ...
                           (local-rep-missing-violations request st)
                           (engagement-fee-mismatch-violations request st)
                           (ust-idnr-unverified-violations request st)
