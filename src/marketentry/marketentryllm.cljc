(ns marketentry.marketentryllm
  ...
  the DEU public-sector market-entry compliance actor.
...
(defn- propose-submit
  ...
  (let [e (store/engagement db subject)]
    {:summary    (str subject " 向けポータル提出提案"
                      (when e (str " (operator=" (:operator e) ")")))
     :rationale  (if e
                   (str "has-local-rep?=" (:has-local-rep? e)
                        " ust-idnr-verified?=" (:ust-idnr-verified? e)
     ...
     :confidence (if (and e
                          (or (not (:requires-local-rep? e))
                              (:has-local-rep? e))
                          (or (not (:requires-ust-idnr? e))
                              (:ust-idnr-verified? e)))
                   0.9 0.3)}))
