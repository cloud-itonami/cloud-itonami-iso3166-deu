...
(defn -main [& _]
  (let [db (store/seed-db)
        actor (op/build db)]
    (println "== engagement/intake eng-1 (DEU, clean) ==")
    ...
    (println "== jurisdiction/assess eng-4 (sets up local-rep-missing) ==")
    ...
    (println "== filing/submit eng-4 (local-rep-missing -> HARD hold) ==")
    (println (exec-op actor "t9" {:op :filing/submit :subject "eng-4"} operator))

    (println "== jurisdiction/assess eng-5 (sets up ust-idnr-unverified) ==")
    ...
    (println "== filing/submit eng-5 (ust-idnr-unverified -> HARD hold) ==")
