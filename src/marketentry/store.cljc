(ns marketentry.store
  "SSoT for the DEU market-entry compliance actor, behind a `Store`
...
(defn demo-data
  ...
  []
  {:engagements
   {"eng-1" {:id "eng-1" :operator "Kita Systems DEU" :portal "e-Vergabe"
             :base-fee 500000 :monthly-rate 30000 :monitoring-months 12
             :claimed-fee 860000.0
             :requires-local-rep? true :has-local-rep? true
             :requires-ust-idnr? true :ust-idnr-verified? true
             :drafted? false :submitted? false
             :jurisdiction "DEU" :status :intake}
    "eng-2" {:id "eng-2" :operator "Atlantis LLC" :portal "e-Vergabe"
             :base-fee 500000 :monthly-rate 30000 :monitoring-months 12
             :claimed-fee 860000.0
             :requires-local-rep? true :has-local-rep? true
             :requires-ust-idnr? true :ust-idnr-verified? true
    ...
    "eng-3" {:id "eng-3" :operator "Minami Systems" :portal "e-Vergabe"
             :base-fee 500000 :monthly-rate 30000 :monitoring-months 12
             ...
             :drafted? false :submitted? false
             :jurisdiction "DEU" :status :intake}
    "eng-4" {:id "eng-4" :operator "Higashi Export" :portal "e-Vergabe"
             :base-fee 500000 :monthly-rate 30000 :monitoring-months 12
             :claimed-fee 860000.0
             :requires-local-rep? true :has-local-rep? false
             :requires-ust-idnr? true :ust-idnr-verified? true
             :drafted? false :submitted? false
             :jurisdiction "DEU" :status :intake}
    "eng-5" {:id "eng-5" :operator "Nishi Logistics" :portal "e-Vergabe"
             :base-fee 500000 :monthly-rate 30000 :monitoring-months 12
             :claimed-fee 860000.0
             :requires-local-rep? true :has-local-rep? true
             :requires-ust-idnr? true :ust-idnr-verified? false
             :drafted? false :submitted? false
             :jurisdiction "DEU" :status :intake}
    "eng-6" {:id "eng-6" :operator "Chuo Civic Tech" :portal "e-Vergabe"
...
(defn- engagement->tx [{:keys [id operator portal base-fee monthly-rate monitoring-months claimed-fee
                               requires-local-rep? has-local-rep?
                               requires-ust-idnr? ust-idnr-verified?
                               drafted? submitted?
  ...
  (cond-> {:engagement/id id}
    ...
    (some? requires-local-rep?)  (assoc :engagement/requires-local-rep? requires-local-rep?)
    (some? has-local-rep?)       (assoc :engagement/has-local-rep? has-local-rep?)
    (some? requires-ust-idnr?)    (assoc :engagement/requires-ust-idnr? requires-ust-idnr?)
    (some? ust-idnr-verified?)    (assoc :engagement/ust-idnr-verified? ust-idnr-verified?)
...
(def ^:private engagement-pull
  [:engagement/id :engagement/operator :engagement/portal :engagement/base-fee :engagement/monthly-rate
   ...
   :engagement/requires-local-rep? :engagement/has-local-rep?
   :engagement/requires-ust-idnr? :engagement/ust-idnr-verified?
...
(defn- pull->engagement [m]
  (when (:engagement/id m)
    {:id (:engagement/id m) :operator (:engagement/operator m) :portal (:engagement/portal m)
     ...
     :requires-local-rep? (boolean (:engagement/requires-local-rep? m))
     :has-local-rep? (boolean (:engagement/has-local-rep? m))
     :requires-ust-idnr? (boolean (:engagement/requires-ust-idnr? m))
     :ust-idnr-verified? (boolean (:engagement/ust-idnr-verified? m))
